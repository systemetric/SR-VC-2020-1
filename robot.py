...
def allMarkersDistances():
    ...
    while repeats > 0:
        ...
        markers = R.see()
        for m in markers:
            if (m.info.code not in list(visible.keys())
                    and m.info.marker_type == MARKER_ARENA and m.dist < 4):
...
def findHome():
    ...
    while visible[0] > 1.3 and visible[27] > 1.3:
        ...
        sorteddist = {
            k: v
            for k, v in sorted(visible.items(), key=lambda item: item[1])
        }
        ...
        print(hr, closest)
        if (closest <= 13 and closest not in sortedkey[0:3]) or (
                closest >= 14 and (closest in sortedkey[-3:-1])):
            print(hr, "going for", off(sortedkey[1]))
            R.seek_marker(off(sortedkey[1]), power=30)
            R.drive_to_marker(off(sortedkey[1]),
                              dist=0.5,
                              power=30,
                              interval=1)
        # elif list(sorteddist.items())[0][0] >= 13:
            ...
            R.drive_to_marker(off(sortedkey[-2]),
                              dist=0.5,
                              power=30,
                              interval=1)
...
def socialDistancing():
    distances = [
        R.left_distance,
        R.right_distance,
        R.back_left_distance,
        R.back_right_distance,
        R.front_left_distance,
        R.front_right_distance,
    ]
...
for i in range(1):
    markers = R.see()
    for m in markers:
        if m.info.marker_type == MARKER_TOKEN_SILVER and m.info.code in [
                47, 45, 46, 44
        ]:
...
for i in range(20):
    markers = R.see()
    for m in markers:
        if m.info.marker_type == MARKER_ARENA and m.info.code in [
                24, 3, 10, 17
        ]:
...
for i in range(1):
    markers = R.see()
    for m in markers:
        if m.info.marker_type == MARKER_TOKEN_GOLD and m.info.code in [
                36, 37, 38, 39
        ]:
    ...
    while got == False:
        ...
        while goken == None:
            ...
            while repeat < 25 and goken == None:
                markers = R.see()
                for m in markers:
                    if m.info.code in range(
                            32, 36) and m.info.code not in collected:
                        g = g + 1
                        goken = m.info.code
                        gm = m
                        break
        ...
        R.setDriveMotors(30)
        time = 0
        while (R.front_right_distance > 0.2
               and R.front_left_distance > 0.2) and time < 30:
